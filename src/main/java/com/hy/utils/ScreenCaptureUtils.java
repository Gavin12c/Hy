...
public class ScreenCaptureUtils {
    ...
    public static BufferedImage getScreenshot(Rectangle bounds) {
        HDC windowDC = GDI.GetDC(USER.GetDesktopWindow()); //窗口句柄-deviceContext Handle
        HBITMAP outputBitmap = GDI.CreateCompatibleBitmap(windowDC,
                bounds.width, bounds.height);//得到兼容位图（DDB）的句柄
        try {
            HDC blitDC = GDI.CreateCompatibleDC(windowDC);//得到内存DC的句柄
            try {
                HANDLE oldBitmap = GDI.SelectObject(blitDC, outputBitmap); //outputBitmap对象指定到DC(blitDC)
                try {
...
interface GDI32 extends com.sun.jna.platform.win32.GDI32 {
    ...
    int SRCCOPY = 0xCC0020;//复制图块的光栅操作码
}
